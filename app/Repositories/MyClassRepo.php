...
class MyClassRepo
{
    ...
    public function all()
    {
        ...
        // return MyClass::groupBy('class_type_id')
        // ->select('class_type_id', \DB::raw('count(*) as total'))
        // ->get();
    }
    // Group MyClass records by class_type_id and count the number of classes per type
    public function getClassesGroupedByType()
    {
        $classTypes = MyClass::groupBy('class_type_id')
        ->select('class_type_id', \DB::raw('count(*) as total'))
        ->get();
            // For each class type, get the full records
        foreach ($classTypes as $classType) {
            $classType->classes = MyClass::where('class_type_id', $classType->class_type_id)->get();
        }
    ...
    {
        return MyClass::groupBy('class_type_id')
        ->select('class_type_id', \DB::raw('GROUP_CONCAT(name) as names'))
        ->get();
    }
