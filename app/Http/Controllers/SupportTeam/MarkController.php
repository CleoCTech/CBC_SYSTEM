...
class MarkController extends Controller
{
    ...
    public function show($student_id, $year)
    {
        ...
        $markSetup = MarkSetup::first();
        $d['markSetup'] = $markSetup;
        // dd($d['subjects']);
        $d['year'] = $year;
    ...
    public function manage($exam_id, $class_id, $subject_id)
    {
        ...
        $d['marks'] = $this->exam->getMark($d);
         ...
         // Ensure $d['marks'] is a collection
        if (!($d['marks'] instanceof \Illuminate\Support\Collection)) {
            $d['marks'] = collect($d['marks']);
        }
        $intClassID = intval($class_id);

        // Filter marks by my_class_id
        // $d['marks'] = $d['marks']->filter(function ($mark) use ($intClassID) {
        //     return intval($mark->my_class_id) === $intClassID;
        // });

        $d['marks'] = $d['marks']->filter(function ($mark) use ($intClassID) {
            //logger('Filtering mark with my_class_id: ' . $mark->my_class_id . ' against ' . $intClassID);
            return intval($mark->user->student_record->my_class_id) === $intClassID;
        });
    ...
    public function updateSetup(Request $request)
    {
        ...
        $request->validate([
            'ca1' => 'required|integer|min:0',
            'ca2' => 'required|integer|min:0',
            'exams' => 'required|integer|min:0',
    ...
    {
        // Fetch the grading system
        $gradingSystems = GradingSystem::all();

        // Determine the grade based on the final average
        foreach ($gradingSystems as $gradingSystem) {
            if ($finalAverage >= $gradingSystem->min_points && $finalAverage <= $gradingSystem->max_points) {
                return [
                    'grade' => $gradingSystem->grade,
                    'remark' => $gradingSystem->remark,
                ];
            }
        }

        // Default return if no grade is found
        return [
            'grade' => 'N/A',
            'remark' => 'Not Graded',
        ];
